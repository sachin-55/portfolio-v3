...
const App: React.FC = () => {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'ai'; content: string }[]>([
    { role: 'ai', content: "Hi! I'm Sachin's AI assistant. Ask me anything about his projects, experience, or skills!" }
  ]);
    ...
    <div className="min-h-screen font-sans selection:bg-purple-600">
      ...
      <nav className="fixed top-0 w-full z-50 bg-black/80 backdrop-blur-md border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-2 group cursor-pointer" onClick={() => window.scrollTo(0, 0)}>
            ...
            <span className="font-bold text-xl tracking-tighter">Bhattarai</span>
          </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="max-w-3xl">
            ...
            <h1 className="text-6xl md:text-8xl font-black mb-8 leading-[0.9] tracking-tighter">
              BUILDING <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-red-500 to-yellow-500">DIGITAL</span> EXPERIENCES.
            </h1>
            <p className="text-xl md:text-2xl text-gray-400 mb-10 leading-relaxed max-w-2xl font-light">
              Fullstack Engineer & Tech Leader specializing in high-performance web and mobile applications. 
              Converting complex problems into elegant code.
            </p>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <div className="flex items-center gap-2">
                {[
                  { icon: <Github className="w-5 h-5" />, href: `https://${PERSONAL_INFO.github}` },
                  { icon: <Linkedin className="w-5 h-5" />, href: `https://${PERSONAL_INFO.linkedin}` },
                  { icon: <Mail className="w-5 h-5" />, href: `mailto:${PERSONAL_INFO.email}` }
                ].map((social, i) => (
                  <a 
                    key={i}
                    href={social.href} 
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="p-6 rounded-3xl bg-black border border-white/5">
                ...
                <div className="text-sm text-gray-500 uppercase tracking-wider font-bold">Years Web Dev</div>
              </div>
              <div className="p-6 rounded-3xl bg-black border border-white/5">
                <div className="text-3xl font-bold text-green-400 mb-1">2+</div>
                <div className="text-sm text-gray-500 uppercase tracking-wider font-bold">Years Mobile</div>
              </div>
            </div>
          </div>
            ...
            <div className="relative aspect-square rounded-[36px] overflow-hidden border border-white/10 bg-black flex items-center justify-center">
              ...
              <div className="absolute bottom-8 left-8">
                <div className="flex items-center gap-2 mb-2">
                   <div className="px-2 py-1 rounded bg-purple-600 text-[10px] font-bold">REACT EXPERT</div>
                   <div className="px-2 py-1 rounded bg-red-600 text-[10px] font-bold">NODE.JS</div>
                </div>
                <div className="text-xl font-bold">Crafting Clean Architectures</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="experience" className="py-20 px-6">
        <div className="max-w-7xl mx-auto">
          <SectionTitle icon={<Briefcase />} title="Experience" subtitle="My professional journey and growth" />
          
          <div className="space-y-4">
            {EXPERIENCE.map((exp, idx) => (
              <div key={idx} className="group relative">
                <div className={`p-8 rounded-[32px] bg-white/5 border border-white/10 transition-all hover:bg-white/[0.08] ${exp.isCurrent ? 'border-purple-500/30 ring-1 ring-purple-500/20' : ''}`}>
                  ...
                  {exp.technologies && (
                    <div className="flex flex-wrap gap-2">
                      {exp.technologies.map((tech, i) => (
                        <span key={i} className="px-3 py-1 rounded-full bg-black border border-white/10 text-xs font-mono text-purple-400">
                          {tech}
                        </span>
                      ))}
                    </div>
      ...
      <section id="projects" className="py-20 px-6 bg-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
            <SectionTitle icon={<Layers />} title="Featured Projects" subtitle="A showcase of digital products I've engineered" />
              ...
              {(['Work', 'Personal'] as const).map(tab => (
                <button
                  ...
                  className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${
                    activeTab === tab ? 'bg-purple-600 text-white shadow-lg shadow-purple-500/20' : 'text-gray-500 hover:text-white'
                  }`}
                >
                  {tab}
                </button>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {PROJECTS.filter(p => p.category === activeTab).map((proj, idx) => (
              <div key={idx} className="group flex flex-col h-full bg-black rounded-[40px] border border-white/5 overflow-hidden transition-all hover:border-purple-500/50 hover:-translate-y-2">
                <div className="aspect-[16/10] bg-gradient-to-br from-white/10 to-transparent relative p-8 flex items-end">
                   ...
                   <div className="relative z-10">
                      <div className="text-xs font-mono text-gray-500 mb-1 uppercase tracking-widest">{proj.period}</div>
                      <h3 className="text-2xl font-bold group-hover:text-purple-400 transition-colors">{proj.title}</h3>
                   </div>
                ...
                <div className="p-8 flex flex-col flex-grow">
                  ...
                  <div className="mt-auto pt-6 border-t border-white/5">
                    <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Core Contribution</h4>
                    <p className="text-sm text-gray-300 italic">"{proj.responsibilities}"</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section id="skills" className="py-20 px-6">
        <div className="max-w-7xl mx-auto">
          <SectionTitle icon={<Code2 />} title="Tech Stack" subtitle="The weapons of my choice" />
          
          <div className="grid md:grid-cols-3 gap-8">
            {SKILLS.map((cat, idx) => (
              <div key={idx} className="p-8 rounded-[40px] bg-white/5 border border-white/10 hover:border-purple-500/30 transition-all">
                <div className="flex items-center gap-3 mb-8">
                  {idx === 0 ? <Cpu className="w-6 h-6 text-yellow-400" /> : 
                   idx === 1 ? <Database className="w-6 h-6 text-green-400" /> : 
                   <Layers className="w-6 h-6 text-red-400" />}
                  <h3 className="text-xl font-bold">{cat.title}</h3>
                </div>
                <div className="flex flex-wrap gap-2">
                  {cat.skills.map((skill, i) => (
                    <span key={i} className="px-4 py-2 rounded-2xl bg-black border border-white/5 text-sm text-gray-300 hover:border-purple-500/50 hover:text-white transition-colors cursor-default">
                      {skill}
                    </span>
                  ))}
                </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-16 mb-20">
            <div>
              ...
              <div className="p-8 rounded-[40px] bg-white/5 border border-white/10">
                <h3 className="text-xl font-bold mb-2 text-yellow-400">{EDUCATION.degree}</h3>
                <p className="text-gray-300 mb-1">{EDUCATION.institution}</p>
                <p className="text-sm font-mono text-gray-500">{EDUCATION.year}</p>
              </div>
            </div>
            <div>
              <SectionTitle icon={<Mail />} title="Get in Touch" />
              <div className="space-y-4">
                <a href={`mailto:${PERSONAL_INFO.email}`} className="flex items-center gap-4 p-6 rounded-[32px] bg-white/5 border border-white/10 hover:border-purple-500 transition-all group">
                  <div className="p-3 bg-purple-500/20 rounded-2xl text-purple-400 group-hover:scale-110 transition-transform">
                    <Mail className="w-6 h-6" />
                  </div>
                  <div>
                    <div className="text-xs text-gray-500 font-bold uppercase tracking-widest mb-1">Email Me</div>
                    <div className="text-lg font-bold">{PERSONAL_INFO.email}</div>
                  </div>
                </a>
                <a href={`tel:${PERSONAL_INFO.phone.replace(/\s+/g, '')}`} className="flex items-center gap-4 p-6 rounded-[32px] bg-white/5 border border-white/10 hover:border-green-500 transition-all group">
                  <div className="p-3 bg-green-500/20 rounded-2xl text-green-400 group-hover:scale-110 transition-transform">
                    <Phone className="w-6 h-6" />
                  </div>
                  <div>
                    <div className="text-xs text-gray-500 font-bold uppercase tracking-widest mb-1">Call Me</div>
                    <div className="text-lg font-bold">{PERSONAL_INFO.phone}</div>
                  </div>
                </a>
              </div>
            </div>
          ...
          <div className="pt-10 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="text-sm text-gray-500">
              © {new Date().getFullYear()} Sachin Bhattarai. All rights reserved.
            </div>
            <div className="flex items-center gap-6">
               <a href={`https://${PERSONAL_INFO.github}`} className="text-gray-500 hover:text-white transition-colors">Github</a>
               <a href={`https://${PERSONAL_INFO.linkedin}`} className="text-gray-500 hover:text-white transition-colors">LinkedIn</a>
            </div>
          </div>
        </div>
      </section>
      ...
      {isChatOpen && (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-end md:p-10 pointer-events-none">
          <div className="w-full md:w-[450px] md:max-h-[700px] h-[80vh] bg-black border border-white/10 shadow-2xl rounded-t-[32px] md:rounded-[40px] flex flex-col overflow-hidden pointer-events-auto">
            ...
            {/* Chat Footer */}
            <form onSubmit={handleSendMessage} className="p-6 border-t border-white/10 flex gap-3">
              <input 
                value={chatInput}
